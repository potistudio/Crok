...
export class HaikuDetector {
	 ...
	 */
	private tokenizeWithMorae(
		text: string
	): Array<{ surface: string; reading: string; morae: number }> {
		if (!this.tokenizer) {
	...
	async detect(text: string): Promise<HaikuMatch | null> {
		...
		if (cleanedText.length > 24) {
			if (this.debug) {
				console.log(
					`${HaikuDetector.COLORS.GRAY}  [DEBUG] 文字数超過 (${cleanedText.length} > 24) のため無効${HaikuDetector.COLORS.RESET}`
				);
			}
			return null;
		}
		...
		if (this.hasUnpairedSymbols(processedText)) {
			if (this.debug) {
				console.log(
					`${HaikuDetector.COLORS.RED}  [DEBUG] 未ペアの記号が含まれているため無効: "${processedText}"${HaikuDetector.COLORS.RESET}`
				);
			}
			return null;
		}
		...
		if (this.debug) {
			...
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}┌────────────────┬────────────────┬──────┐${HaikuDetector.COLORS.RESET}`
			);
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET} 表層形         ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET} 読み           ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET}モーラ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET}`
			);
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}├────────────────┼────────────────┼──────┤${HaikuDetector.COLORS.RESET}`
			);
			for (const token of tokens) {
				...
				console.log(
					`  ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET} ${surface} ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET} ${reading} ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.YELLOW}${morae}  ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET}`
				);
			}
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}└────────────────┴────────────────┴──────┘${HaikuDetector.COLORS.RESET}`
		...
		if (this.debug) {
			console.log(
				`${HaikuDetector.COLORS.BLUE}  [DEBUG] 元テキスト文字数（空白・句読点除く）: ${originalLength}${HaikuDetector.COLORS.RESET}`
			);
		}
		...
		while (startIdx < tokens.length) {
			...
			if (match) {
				...
				if (this.debug) {
					console.log(
						`${HaikuDetector.COLORS.MAGENTA}  [DEBUG] 候補発見 (startIdx=${startIdx}): "${match.text}"${HaikuDetector.COLORS.RESET}`
					);
					console.log(
						`  [DEBUG]   俳句文字数: ${haikuLength}, 元文字数: ${originalLength}`
					);
				}
				...
				if (haikuLength === originalLength) {
					if (this.debug) {
						console.log(
							`${HaikuDetector.COLORS.GREEN}${HaikuDetector.COLORS.BOLD}  [DEBUG]   → 有効な俳句として採用${HaikuDetector.COLORS.RESET}`
						);
					}
					// 俳句を返す（スキャン終了）
					return match;
					...
					if (this.debug) {
						console.log(
							`${HaikuDetector.COLORS.RED}  [DEBUG]   → 文字数不一致のため無効${HaikuDetector.COLORS.RESET}`
						);
					}
				}
			}
	...
	private tryMatchHaiku(
		...
		if (!naka) {
			if (this.debug) {
				console.log(
					`  ${HaikuDetector.COLORS.GRAY}└── [DEBUG] 中の句(7音)不成立 → 次のスキャン位置: ${kami.endIdx + 1}${HaikuDetector.COLORS.RESET}`
				);
			}
		...
		if (!shimo) {
			if (this.debug) {
				console.log(
					`  ${HaikuDetector.COLORS.GRAY}└── [DEBUG] 下の句(5音)不成立 → 次のスキャン位置: ${naka.endIdx + 1}${HaikuDetector.COLORS.RESET}`
				);
			}
	 ...
	 */
	private static readonly INVALID_SYMBOL_PATTERN =
		/[#@:/\\<>{}[\]()（）「」『』【】、。，．,.!！?？;；:：'"'""`・\uD800-\uDFFF]|https?|www\./;
	...
	private findPhrase(
		...
		if (this.debug) {
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}├──${HaikuDetector.COLORS.RESET} 🔍 ${HaikuDetector.COLORS.CYAN}${HaikuDetector.COLORS.BOLD}${phraseName}(${targetMorae}音)${HaikuDetector.COLORS.RESET} 探索開始 (index: ${startIdx})`
			);
		}
		...
		while (actualStartIdx < tokens.length) {
			...
			if (/^[\s\u3000、。！？!?,.]+$/.test(token.surface)) {
				if (this.debug) {
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   ├── [${actualStartIdx}] "${token.surface}" → Skip (句頭記号)${HaikuDetector.COLORS.RESET}`
					);
				}
				actualStartIdx++;
			} else {
		...
		for (let i = actualStartIdx; i < tokens.length; i++) {
			...
			if (/^[\s\u3000、。！？!?,.]+$/.test(token.surface)) {
				if (this.debug) {
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   ├── ${HaikuDetector.COLORS.RED}❌ [${i}] "${token.surface}" : 句読点割り込み${HaikuDetector.COLORS.RESET}`
					);
				}
				return null;
			}
			...
			if (!isNumberToken && HaikuDetector.INVALID_SYMBOL_PATTERN.test(token.surface)) {
				if (this.debug) {
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   ├── ${HaikuDetector.COLORS.RED}❌ [${i}] "${token.surface}" : 禁止記号${HaikuDetector.COLORS.RESET}`
					);
				}
				return null;
			}
			...
			const isDiphthongMatch =
				currentMorae === targetMorae + 1 && this.endsWithDiphthong(combinedReading);

			if (isMatch || isDiphthongMatch) {
				if (this.debug) {
					// 最後のトークンの累積ログも出力
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   ├── [${i}] "${token.surface}" : 累積「${combinedReading}」= ${currentMorae}音${HaikuDetector.COLORS.RESET}`
					);
					...
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│${HaikuDetector.COLORS.RESET}   ${HaikuDetector.COLORS.GREEN}${HaikuDetector.COLORS.BOLD}✨ ${phraseName}検出: "${text}"${HaikuDetector.COLORS.RESET} (計${currentMorae}音)${extraInfo}`
					);
				}
				...
				if (nextToken && !/^[\s\u3000、。！？!?,.]+$/.test(nextToken.surface)) {
					const nextReading = this.hiraganaToKatakana(
						nextToken.reading || nextToken.surface
					);
					const firstChar = [...nextReading][0];
					if (firstChar && "ーンッ".includes(firstChar)) {
						if (this.debug) {
							console.log(
								`  ${HaikuDetector.COLORS.GRAY}│       ⚠️ 次トークン"${nextToken.surface}"(${firstChar})をマージ必要${HaikuDetector.COLORS.RESET}`
							);
						}
						continue;
					}
			...
			if (currentMorae > targetMorae + 1) {
				if (this.debug) {
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   └── ${HaikuDetector.COLORS.RED}❌ [${i}] "${token.surface}" : モーラ超過 (${currentMorae} > ${targetMorae})${HaikuDetector.COLORS.RESET}`
					);
				}
				return null;
			}
			...
			if (currentMorae === targetMorae + 1 && !this.endsWithDiphthong(combinedReading)) {
				if (this.debug) {
					console.log(
						`  ${HaikuDetector.COLORS.GRAY}│   └── ${HaikuDetector.COLORS.RED}❌ [${i}] "${token.surface}" : モーラ超過・二重母音なし (${currentMorae} > ${targetMorae})${HaikuDetector.COLORS.RESET}`
					);
				}
				return null;
			}
		...
		if (this.debug) {
			console.log(
				`  ${HaikuDetector.COLORS.GRAY}│   └── ⚠️ トークン終端により未完成${HaikuDetector.COLORS.RESET}`
			);
		}
		return null;
	}
