...
describe("SimplifierService", () => {
			...
			try {
				...
				if (shouldSimplify) {
					...
					if (typeof result === "string") {
						...
						console.warn(
							`[WARN] Expected simplification for "${input.substring(0, 20)}..." but got null. AI might have declined.`
						);
					}
				} else {
				...
				console.warn(
					`[WARN] Error processing "${input.substring(0, 20)}..." (API might be down/invalid):`,
					e
				);
	...
	it.runIf(apiKeyExists)("should force simplify when force flag is true", async () => {
		const text =
			"昨日は美味しいお寿司を食べました。とても新鮮で、特にマグロが最高でした。また行きたいです。";
		const result = await simplifierService.simplify(text, true);

		if (result === null) {
			console.warn(
				"[WARN] Forced simplification returned null. Check API key or Service logic."
			);
		} else {
